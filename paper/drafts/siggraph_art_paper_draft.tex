...
Participants enter the installation by scanning a QR code, which binds their session to a unique identifier stored in a Supabase backend. A pre-experience survey captures demographic information and the participant's existing perception of y\=okai through a forced-choice item with options including ``entertainment character,'' ``cultural tradition,'' ``psychological phenomenon,'' and ``something I rarely think about.'' This survey provides the baseline against which post-experience shifts are measured.
...
\subsection{Pre-Post Perceptual Measurement}

Participants complete iPad-based surveys upon entry (pre) and exit (post). The pre-survey records the participant's existing y\=okai perception through a forced-choice item. The post-survey asks ``What was this artwork about?'' as a free-text response, followed by behavioral intention items assessing whether the experience prompted interest in investigating local folklore or sharing the encounter with others. The shift between pre-perception and post-interpretation constitutes the primary quantitative measure.

\subsection{Deductive Coding Framework}

Free-text responses are analyzed using a five-category coding framework aligned with the research question.

\begin{itemize}
  \item \textbf{C1 (Character Consumption):} The participant describes y\=okai as entertainment or collectible content. This indicates no perceptual shift.
  \item \textbf{C2 (Technology Focus):} The participant focuses on the novelty of the AI system or the visual quality of the output. This indicates engagement with the medium rather than the cultural content.
  \item \textbf{C3 (Spatial or Cultural Anchoring):} The participant references place, memory, or local tradition. This indicates partial engagement with the cultural substrate.
  \item \textbf{C4 (Anxiety Externalization):} The participant describes the experience as confronting, naming, or externalizing a personal fear. This indicates core engagement with the y\=okai generation mechanism.
  \item \textbf{C5 (Ephemeral Reflection):} The participant references the fading ink, the transience of the receipt, or the lifecycle of memory. This indicates engagement with the materiality design.
\end{itemize}

A predominance of C3, C4, and C5 codes across participants would support an affirmative answer to the research question.
...
The evaluation framework is designed to determine whether this structural correspondence produces measurable consequences for participant perception. If participants predominantly produce C1 (character consumption) or C2 (technology focus) codes in the post-experience survey, the system has failed to activate the cultural mechanism it reconstructs. If participants predominantly produce C3, C4, or C5 codes, the system has successfully shifted perception toward the generative practice.
